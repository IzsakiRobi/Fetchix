...
public class DownloadRow : Gtk.ListBoxRow {
    ...
    public void update_status(string status, int64 completed, int64 total, int64 speed, int64 eta) {
        ...
        if (status == "complete") {
            ...
            if (display_tot > 0) {
                status_label.set_text("Paused - %s of %s".printf(format_bytes(display_comp), format_bytes(display_tot)));
            } else {
            ...
            string eta_str = (eta > 0) ? " - " + format_time(eta) + " left" : "";
            
            if (display_tot > 0) {
                status_label.set_text("%s of %s at %s/s%s".printf(format_bytes(display_comp), format_bytes(display_tot), format_bytes(speed), eta_str));
            } else {
                status_label.set_text("%s downloaded at %s/s".printf(format_bytes(display_comp), format_bytes(speed)));
            }
        }
    }
}
...
public class SettingsDialog : Adw.Window {
    ...
    public SettingsDialog(FetchixApp app, Gtk.Window parent, AppSettings app_settings) {
        GLib.Object(transient_for: parent, modal: true);
        this.main_app = app;
        this.settings = app_settings;
        
        this.title = "Preferences";
        this.set_default_size(450, 420);
        ...
        cancel_btn.clicked.connect(() => { this.destroy(); });
...
public class FetchixApp : Adw.Application {
    ...
    protected override void activate() {
        ...
        pref_btn.clicked.connect(() => {
            var settings_dialog = new SettingsDialog(this, window, settings);
        ...
        header.set_title_widget(url_entry);
